	...
	do 
		...
		do 
			...
			do 
				Messages = {
					...
					["blaze"] = {
						"Owned By blaze",
						"blaze blaze blaze RAHHHHH",
						"Slammed by blaze",
						"YOU GOT SLAMMED BY blaze",
						"SEEMS LIKE YOU NEED blaze GG/blazeVIP",
						"GG/blazeVIP GG/blazeVIP GG/blazeVIP",
						"GG/blazeVIP <-- THIS WILL LET YOU COPE WITH YOUR ISSUES",
						"WHAT YOU CANT BEAT blaze?",
						"PRO TIP: BUY blaze",
					},
				}
				utx = {
		...
		do 
			...
			function HitReg:HitSound(Id, Volume, Pitch)
				...
				--[[writefile("song.mp3", game:HttpGet("https://blaze.feet.media/osiris/cmwr7ecb.mp3"))
...
local Window = Library:Window({Size = UDim2.new(0,580,0,600)}) do 
	...
	local misc = Window:Page({Name = "Misc", Weapons = true}) do 
		...
		local chatSpam = Movement:Section({Name = "Chat Spam", Size = 330, Side = "Left"}) do
			...
			chatSpam:Dropdown({Name = "Type", Flag = "Chat Spam Type", Options = {"blaze", "Scottish"}, Default = "blaze"})
	...
	local Config = Window:Page({Name = "Settings"}) do
		...
		--
		local CurrentList = {};
		if not isfolder("blaze") then 
			makefolder("blaze")
		end 
		if not isfolder("blaze/Configs") then 
			makefolder("blaze/Configs")
		end 
		local function UpdateConfigList()
			local List = {};
			for idx, file in ipairs(listfiles("blaze/Configs")) do
				local FileName = file:gsub("blaze/Configs\\", ""):gsub(".cfg", "")
		...
		Cfgs:Button({Name = "Create", Callback = function()
			...
			if ConfigName == "" or isfile("blaze/Configs/" .. ConfigName .. ".cfg") then
				return;
			end;
			writefile("blaze/Configs/" .. ConfigName .. ".cfg", Library:GetConfig());
			UpdateConfigList();
		end})
		Cfgs:Button({Name = "Save", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				writefile("blaze/Configs/" .. SelectedConfig .. ".cfg", Library:GetConfig())
			end;
		end})
		Cfgs:Button({Name = "Load", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				Library:LoadConfig(readfile("blaze/Configs/" .. SelectedConfig .. ".cfg"))
			end
		end})
		Cfgs:Button({Name = "Delete", Callback = function()
			local SelectedConfig = flags.SettingConfigurationList;
			if SelectedConfig then
				delfile("blaze/Configs/" .. SelectedConfig .. ".cfg")
			end
			UpdateConfigList();
		end})
